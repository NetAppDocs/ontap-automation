...
[NOTE]
All the examples create and modify roles using `/api/security/roles` and the derived REST endpoints. In each case, the curl command refers to a separate JSON input file for clarity.
...
The same role can be modified to allow the user to create a volume clone.
