...
A set of one or more parameters can be appended to the URL string beginning after the `?` character. If more than one parameter is provided, the query parameters are split using the `&` character. Then each key and value in the parameter are split at the `=` character.

For example, you can specify an exact value to match using only the equal sign:
...
+
Filtering based on the `state` variable to select spare disks.
