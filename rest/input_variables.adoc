...
A set of one or more parameters is appended to the URL string beginning after the `?` character. If more than one parameter is provided, the query parameters are split with the `&` character. Then each key and value in a pair are split at the `=` character.

For example, you can specify an exact value to match using only the equal sign as follows:

`<field>=<value>`

For a more complex query, the additional operator is placed after the equal sign. For example, to select objects with a field that is greater than or equal to a specific value, this would be:
