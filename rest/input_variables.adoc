...
[TIP]
If zero records are returned for an API call, with no next link, this might indicate that a timeout occurred before any records could be fetched. In this case, you can retry the request using a larger `return_timeout` value or a smaller `max_records` value.
