...
|Description
|REST
|The REST roles were introduced with ONTAP 9.6 and are generally applied to users accessing ONTAP through the REST API. They evolved to also support CLI access.
|Traditional
...
|SVM
|Roles with an SVM scope are defined for a specific data SVM. They can be associated with user accounts in the same SVM.
|===
