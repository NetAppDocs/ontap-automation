...
[TIP]
You can include the user without a role. In this case, the user is assigned a default role (`admin` or `vsadmin`) depending on whether the user is created with a cluster or SVM scope. You’ll need to modify the user to assign a different role.
