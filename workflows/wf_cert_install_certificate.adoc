---
sidebar: sidebar
permalink: workflows/wf_cert_install_certificate.html
keywords: workflow, install certificat
summary: 
---

= Install a certificate
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
:imagesdir: ./media/

[.lead]
You can install a certificate in your ONTAP cluster. There are several reasons you might do this, such as to configure a web server or implement OAuth 2.0 client authentication and authorization.

.Before you begin

You must have the certificate or certificates you want to install. If a certificate is signed by a root CA (that is, not self-signed), you need to make sure the root CA certificate and any intermediary certificates are install in ONTAP before using it.

.HTTP method and endpoint

This REST API call uses the following method and endpoint.

[cols="25,75"*,options="header"]
|===
|HTTP method
|Path
|POST
|/api/security/certificates
|===

.Curl example: Install a self-signed root CA cluster certificate

[source,curl]
curl --request POST \
--location "https://$FQDN_IP/api/security/certificates" \
--include \
--header "Content-Type: application/json" \
--header "Accept: */*" \
--header "Authorization: Basic $BASIC_AUTH" \
--data @JSONinput

.JSON input example

[source,json]
----
{
    "type": "server_ca",
    "public_certificate":
    "-----BEGIN CERTIFICATE-----

IpYuBcuKXLwAarhDEacXttVjC+Bq
-----END CERTIFICATE-----"
}
----