...
You must have the certificate you want to install. If the certificate is not self-signed, you should make sure the root CA certificate and any intermediate certificates are also installed before using it.

Before using the JSON input example, make sure to update the variable `public_certificate` with the certificate for your environment.
