...
The primary authentication method available for the REST API is HTTP basic authentication. Beginning with ONTAP 9.14, you also have the option of using the Open Authorization (OAuth 2.0) framework with token-based authentication and authorization.
...
. Plan xxx (assess env)
. Install cert, etc.
. Ddefine the authorization server to ONTAP
. Gloablly enable OAuth 2.0 in the ONTAP cluster

.Request an access token

xxx
>> Outside of ONTAP
>> Different techniques, such as issuing curl
...
`Authorization: Bearer YWRtaW46ZGF2aWQxMjM= ...`
