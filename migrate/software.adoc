...
The ONTAPI reporting tool is designed to help NetApp professional services, customers, and partners identify the ONTAPI usage in their ONTAP environment. Scripts are provided for three different use cases as shown in the table below.
...
The Python client library is a package you can install and use to access the ONTAP REST API. It allows you to quickly develop robust code for ONTAP automation.

For an introduction to the Python client library, see link:../python/overview_pcl.html[Overview of the Python client library]. For the Python samples see https://github.com/NetApp/ontap-rest-python/tree/master/examples/python_client_library[PCL examples^].
