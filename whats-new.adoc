...
summary: "Each ONTAP release updates the ONTAP REST API to bring you new features, enhancements, and bug fixes."
---
...
[TIP]
You should review the https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508[ONTAP Release Notes^] for additional information including known limitations or issues. Also see link:api-changes.html[Changes to ONTAP REST API calls] for any changes that might impact your automation software.

== ONTAP 9.19.1

The ONTAP 9.19.1 release includes several updates which continue to enhance and refine the ONTAP REST API.

[.big red]*IPSec and OIDC (Check SM UI)*

Security::
A new `whoami` feature has been added to retrieve the username, role, and permissions information for the logged in user. The web authentication (WebAuthn) capability has also been enhanced. IPsec has been updated to retrieve the policies and security associations used with the cluster network.

SAN::
LUN map administration has been updated with an improved update capability.

== ONTAP 9.18.1

The ONTAP 9.18.1 release continues to expand the capabilities of the ONTAP REST API with thirteen new API calls. The enhancements focus primarily on security but also include an improvement related to storage administration.

Security::
Several security enhancements have been introduced with the ONTAP 9.18.1 REST API. You can use the new cluster network endpoints to display and update the cluster network security configuration including certificate management. In addition, you can administer the security configuration of the HA network for NVLog traffic. Autonomous Ransomware Protection (ARP) has been enhanced to support the display and update of the auto enablement configuration.

Storage::
NetApp FlexCache is a persistent cache of an origin volume. It supports fan-out so that more than one FlexCache can be created from a single origin volume. The REST API includes support for the retrieval of the connection status between the cache instances and origin volumes.

== ONTAP 9.17.1

The ONTAP 9.17.1 release continues to expand the capabilities of the ONTAP REST API with nearly two dozen new API calls. This release focuses primarily on security enhancements with additional updates to mediator processing, NMVe subsystems, and application containers.

Security::
There are four major security features introduced with the ONTAP 9.17.1 REST API. ONTAP just in time (JIT) privilege elevation is an enhancement to role-based access control (RBAC). Cluster administrators can request temporary elevation to an existing role which allows access to privileged commands. The feature includes a robust set of configuration options. Support for the OpenStack Barbican key manager has been added. This KMS is used to maintain keys for NetApp Volume Encryption (NVE). Autonomous Ransomware Protection (ARP) for SAN volumes is included with ONTAP 9.17.1. Detailed ARP entropy statistics are available through the REST API. Several API calls have also been added to manage the SAML default metadata configuration for a cluster.

Cluster mediators::
You can ping the NetApp Console cloud service using the ONTAP REST API. You can also modify the configuration of a specific mediator using the PATCH method with an existing endpoint.

NVMe::
NVMe subsystems maintain the configuration state and namespace access control for a set of NVMe-connected hosts. With ONTAP 9.17.1, you can modify an NVMe subsystem using the PATCH method with an existing endpoint.

Application containers::
Application containers are new with ONTAP 9.17.1 and enable provisioning of one or more storage objects. You can configure the policies and rules needed for client access to storage. FlexCache volumes can also be provisioned.
...
[NOTE]
The ONTAP REST API exposed to users of NetApp ASA r2 systems (ASA A1K, ASA A70, and ASA A90) is different than the REST API provided with all other FAS, AFF, and ASA systems. See https://docs.netapp.com/us-en/asa-r2/learn-more/rest-api-support.html[REST API support for ASA r2 systems^] for more information. 
...
+
* Group UUID to group name mapping (`/security/groups`)
* UUID group to role mapping (`/security/group/role-mapping`)
