...
Security::
There are four major security features introduced with the ONTAP 9.17.1 REST API. ONTAP just in time (JIT) privilege elevation is an enhancement to role-based access control (RBAC). Cluster administrators can request temporary elevation to an existing role which allows access to privileged commands. The feature includes a robust set of configuration options. Support for the OpenStack Barbican key manager has been added. This KMS is used to maintain keys for NetApp Volume Encryption (NVE). Autonomous Ransomware Protection (ARP) for SAN volumes is included with ONTAP 9.17.1. Detailed ARP entropy statistics are available through the REST API. Several API calls have been added to manage the SAML default metadata configuration for a cluster.

Cluster mediators::
You can ping the BlueXP cloud service. The configuration of a specific mediator can also be modified through support for the PATCH method with an existing endpoint.

NVMe::
NVMe subsystems maintain the configuration state and namespace access control for a set of NVMe-connected hosts. With ONTAP 9.17.1, an NVMe subsystem can be modified through the PATCH method with an existing endpoint.

Application containers::
Application containers are new with ONTAP 9.17.1 and enable provisioning of one or more storage objects. You can configure the policies and rules needed to for client access to storage. FlexCache volumes can also be provisioned.
