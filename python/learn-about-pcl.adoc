...
[.lead]
The NetApp ONTAP Python client library is a package you can install and use to quickly write scripts that access the ONTAP REST API. You'll need to prepare the local runtime environment before using it. You need to use the _pip_ utility to install the netapp_ontap package from the Python Package Index (PyPi) site.
...
== Separate packages by ONTAP release
...
Each ONTAP release beginning with 9.6 has a PyPI package and associated documentation. Installation requirements are included with each package and specify different versions of: python, requests, requests-toolbelt, and marshmallow.
