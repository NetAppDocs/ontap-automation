...
You can use this set of API calls to manage the policies in effect for an IPSec deployment. This resource type is new with ONTAP 9.8. It has been updated to retrieve polices used for the cluster network with ONTAP 9.19.

.IPSec security associations

You can use this set of API calls to manage the security associations in effect for an IPSec deployment. This resource type is new with ONTAP 9.8. It has been updated to retrieve security associations used for the cluster network with ONTAP 9.19.
...
Web authentication (WebAuthn) is a web standard for securely authenticating users based on public key cryptography. With ONTAP, it supports the administration of phishing-resistent MFAs through System Manager and the ONTAP REST API. This feature was added with ONTAP 9.16 and updated with ONTAP 9.19.1.

.Whoami

An endpoint is available to display the details of the authenticated user including username, role, and permissions. This resource type was introduced with ONTAP 9.19.1.