...
First, copy and paste the curl example into the terminal shell. Then completely remove the `--data` parameter and replace it with the `--data-raw` parameter. Finally, copy and paste in the JSON example so that it follows the curl command and updated parameter.
