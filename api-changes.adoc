...
In addition to disruptive changes to existing API calls, there are several additional considerations when using the ONTAP REST API. These involve new behavior or cautions that you should be aware of.

[cols="40%,50%,10%",options="header"]
|===
| Area or endpoint | Description of behaviore | ONTAP release
|`/api/cluster/licensing/licenses`
|When retrieving license information through the REST API on large ONTAP clusters (32 nodes or higher), you may experience slow response times. This performance issue impacts System Manager licensing tabs and automated workflows that query all licenses simultaneously. To reduce the impact, you can fetch licenses for specific features one at a time using query parameters, or through the system license show CLI command, instead of retrieving all licenses in a single API call.
|9.19.1
